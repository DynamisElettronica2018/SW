...
;DPX.c,102 :: 		onCanInterrupt{
;DPX.c,107 :: 		unsigned int dataLen = 0, flags = 0;
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
	...
	MOV	#0, W0
	MOV	W0, [W14+22]
;DPX.c,114 :: 		Can_clearInterrupt();         //la posizione del clear interrup deve essere per forza questa.
	CALL	_Can_clearInterrupt
;DPX.c,115 :: 		dSignalLed_switch(DSIGNAL_LED_RED_RIGHT);
	MOV.B	#1, W10
	CALL	_dSignalLed_switch
;DPX.c,116 :: 		Can_read(&id, dataBuffer, &dataLen, &flags);
	ADD	W14, #22, W3
	ADD	W14, #20, W2
	ADD	W14, #12, W1
	ADD	W14, #8, W0
	...
	MOV	W0, W10
	CALL	_Can_read
;DPX.c,125 :: 		if (dataLen >= 2) {
	MOV	[W14+20], W0
	CP	W0, #2
	BRA GEU	L__CAN_Interrupt49
	GOTO	L_CAN_Interrupt13
L__CAN_Interrupt49:
;DPX.c,126 :: 		firstInt = (unsigned int) ((dataBuffer[0] << 8) | (dataBuffer[1] & 0xFF));
	ADD	W14, #12, W1
	MOV.B	[W1], W0
	ZE	W0, W0
	SL	W0, #8, W2
...
;DPX.c,127 :: 		}
L_CAN_Interrupt13:
;DPX.c,128 :: 		if (dataLen >= 4) {
	MOV	[W14+20], W0
	CP	W0, #4
	BRA GEU	L__CAN_Interrupt50
	GOTO	L_CAN_Interrupt14
L__CAN_Interrupt50:
;DPX.c,129 :: 		secondInt = (unsigned int) ((dataBuffer[2] << 8) | (dataBuffer[3] & 0xFF));
	ADD	W14, #12, W1
	ADD	W1, #2, W0
	MOV.B	[W0], W0
	ZE	W0, W0
...
;DPX.c,130 :: 		}
L_CAN_Interrupt14:
;DPX.c,131 :: 		if (dataLen >= 6) {
	MOV	[W14+20], W0
	CP	W0, #6
	BRA GEU	L__CAN_Interrupt51
	GOTO	L_CAN_Interrupt15
L__CAN_Interrupt51:
;DPX.c,132 :: 		thirdInt = (unsigned int) ((dataBuffer[4] << 8) | (dataBuffer[5] & 0xFF));
	ADD	W14, #12, W1
	ADD	W1, #4, W0
	MOV.B	[W0], W0
	ZE	W0, W0
...
;DPX.c,133 :: 		}
L_CAN_Interrupt15:
;DPX.c,134 :: 		if (dataLen >= 8) {
	MOV	[W14+20], W0
	CP	W0, #8
	BRA GEU	L__CAN_Interrupt52
	GOTO	L_CAN_Interrupt16
L__CAN_Interrupt52:
;DPX.c,135 :: 		fourthInt = (unsigned int) ((dataBuffer[6] << 8) | (dataBuffer[7] & 0xFF));
	ADD	W14, #12, W1
	ADD	W1, #6, W0
	MOV.B	[W0], W0
	ZE	W0, W0
	...
	ADD	W14, #6, W0
	IOR	W2, W1, [W0]
;DPX.c,136 :: 		}                                                        //noi quando mandiamo sw temp e curr?? come lo mostriamo a schermo??
L_CAN_Interrupt16:
;DPX.c,138 :: 		switch (id) {
	GOTO	L_CAN_Interrupt17
;DPX.c,139 :: 		case EFI_GEAR_RPM_TPS_APPS_ID:
L_CAN_Interrupt19:
;DPX.c,140 :: 		dRpm_set(secondInt);
	MOV	[W14+2], W10
	CALL	_dRpm_set
;DPX.c,141 :: 		dEfiSense_heartbeat();
...
L_CAN_Interrupt22:
;DPX.c,160 :: 		dd_Indicator_setFloatValueP(&ind_efi_slip.base, dEfiSense_calculateSlip(thirdInt));
	MOV	[W14+4], W10
...
;DPX.c,161 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,162 :: 		case EFI_FUEL_FAN_H2O_LAUNCH_ID:
L_CAN_Interrupt23:
;DPX.c,163 :: 		dd_Indicator_setIntValueP(&ind_launch_control.base, fourthInt); //è un flag
	MOV	[W14+6], W11
	MOV	#lo_addr(_ind_launch_control), W10
	CALL	_dd_Indicator_setIntValueP
;DPX.c,164 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,165 :: 		case EFI_PRESSURES_LAMBDA_SMOT_ID:
L_CAN_Interrupt24:
;DPX.c,166 :: 		dd_Indicator_setFloatValueP(&ind_fuel_press.base, dEfiSense_calculatePressure(firstInt));
	MOV	[W14+0], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,167 :: 		dd_Indicator_setFloatValueP(&ind_oil_press.base, dEfiSense_calculatePressure(secondInt));
	MOV	[W14+2], W10
	CALL	_dEfiSense_calculatePressure
	MOV	W0, W11
	MOV	W1, W12
...
;DPX.c,168 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,169 :: 		case GCU_CLUTCH_FB_SW_ID:
L_CAN_Interrupt25:
;DPX.c,170 :: 		dClutch_injectActualValue(firstInt, (unsigned char)secondInt);
	MOV.B	[W14+2], W11
	MOV	[W14+0], W10
...
L_CAN_Interrupt34:
;DPX.c,212 :: 		dAcc_feedbackGCU(secondInt);
	MOV	[W14+2], W10
