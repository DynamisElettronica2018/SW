...
;DPX.c,74 :: 		}
L_timer2_interrupt8:
;DPX.c,75 :: 		timer2_counter1 = 0;
	CLR	W0
	MOV	W0, _timer2_counter1
;DPX.c,76 :: 		}
L_timer2_interrupt7:
;DPX.c,78 :: 		if (timer2_counter3 >= 100) {
	MOV	#100, W1
	MOV	#lo_addr(_timer2_counter3), W0
	CP	W1, [W0]
...
;DPX.c,100 :: 		onCanInterrupt{
;DPX.c,105 :: 		unsigned int dataLen = 0, flags = 0;
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
	...
	MOV	#0, W0
	MOV	W0, [W14+22]
;DPX.c,112 :: 		Can_clearInterrupt();         //la posizione del clear interrup deve essere per forza questa.
	CALL	_Can_clearInterrupt
;DPX.c,113 :: 		dSignalLed_switch(DSIGNAL_LED_RED_RIGHT);
	MOV.B	#1, W10
	CALL	_dSignalLed_switch
;DPX.c,114 :: 		Can_read(&id, dataBuffer, &dataLen, &flags);
	ADD	W14, #22, W3
	ADD	W14, #20, W2
	ADD	W14, #12, W1
	ADD	W14, #8, W0
...
;DPX.c,133 :: 		fourthInt = (unsigned int) ((dataBuffer[6] << 8) | (dataBuffer[7] & 0xFF));
	...
	ADD	W14, #6, W0
	IOR	W2, W1, [W0]
;DPX.c,134 :: 		}                                                        //noi quando mandiamo sw temp e curr?? come lo mostriamo a schermo??
L_CAN_Interrupt16:
;DPX.c,136 :: 		switch (id) {
	GOTO	L_CAN_Interrupt17
;DPX.c,137 :: 		case EFI_GEAR_RPM_TPS_APPS_ID:
L_CAN_Interrupt19:
;DPX.c,138 :: 		dRpm_set(secondInt*10);
	MOV	[W14+2], W1
	MOV	#10, W0
	MUL.UU	W1, W0, W0
	MOV	W0, W10
	CALL	_dRpm_set
;DPX.c,139 :: 		dEfiSense_heartbeat();
	CALL	_dEfiSense_heartbeat
;DPX.c,140 :: 		dGear_propagate(firstInt);
	MOV	[W14+0], W10
	CALL	_dGear_propagate
;DPX.c,141 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,142 :: 		case EFI_WATER_TEMPERATURE_ID:
L_CAN_Interrupt20:
;DPX.c,143 :: 		dd_Indicator_setFloatValueP(&ind_th2o_sx_in.base, dEfiSense_calculateWaterTemperature(firstInt));
	MOV	[W14+0], W10
...
;DPX.c,144 :: 		dd_Indicator_setFloatValueP(&ind_th2o_sx_out.base, dEfiSense_calculateWaterTemperature(secondInt));
	MOV	[W14+2], W10
	CALL	_dEfiSense_calculateWaterTemperature
	MOV	W0, W11
	MOV	W1, W12
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,145 :: 		dd_Indicator_setFloatValueP(&ind_th2o_dx_in.base, dEfiSense_calculateWaterTemperature(thirdInt));
	MOV	[W14+4], W10
	CALL	_dEfiSense_calculateWaterTemperature
	MOV	W0, W11
	MOV	W1, W12
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,146 :: 		dd_Indicator_setFloatValueP(&ind_th2o_dx_out.base, dEfiSense_calculateWaterTemperature(fourthInt));
	MOV	[W14+6], W10
	CALL	_dEfiSense_calculateWaterTemperature
	MOV	W0, W11
	MOV	W1, W12
...
;DPX.c,147 :: 		break;//*/
	GOTO	L_CAN_Interrupt18
;DPX.c,148 :: 		case EFI_OIL_T_ENGINE_BAT_ID:
L_CAN_Interrupt21:
;DPX.c,150 :: 		dd_Indicator_setFloatValueP(&ind_oil_temp_in.base, dEfiSense_calculateOilInTemperature(firstInt));
	MOV	[W14+0], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,151 :: 		dd_Indicator_setFloatValueP(&ind_oil_temp_out.base, dEfiSense_calculateOilOutTemperature(secondInt));
	MOV	[W14+2], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,152 :: 		dd_Indicator_setFloatValueP(&ind_th2o.base, dEfiSense_calculateTemperature(thirdInt));
	MOV	[W14+4], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,153 :: 		dd_Indicator_setFloatValueP(&ind_vbat.base, dEfiSense_calculateVoltage(fourthInt));
	MOV	[W14+6], W10
...
;DPX.c,155 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,156 :: 		case EFI_TRACTION_CONTROL_ID:
L_CAN_Interrupt22:
;DPX.c,157 :: 		dd_Indicator_setFloatValueP(&ind_efi_slip.base, dEfiSense_calculateSlip(thirdInt));
	MOV	[W14+4], W10
...
;DPX.c,158 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,159 :: 		case EFI_FUEL_FAN_H2O_LAUNCH_ID:
L_CAN_Interrupt23:
;DPX.c,160 :: 		dd_Indicator_setIntValueP(&ind_launch_control.base, fourthInt); //è un flag
	MOV	[W14+6], W11
	MOV	#lo_addr(_ind_launch_control), W10
	CALL	_dd_Indicator_setIntValueP
;DPX.c,161 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,162 :: 		case EFI_PRESSURES_LAMBDA_SMOT_ID:
L_CAN_Interrupt24:
;DPX.c,163 :: 		dd_Indicator_setFloatValueP(&ind_fuel_press.base, dEfiSense_calculatePressure(firstInt));
	MOV	[W14+0], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,164 :: 		dd_Indicator_setFloatValueP(&ind_oil_press.base, dEfiSense_calculatePressure(secondInt));
	MOV	[W14+2], W10
	CALL	_dEfiSense_calculatePressure
	MOV	W0, W11
	MOV	W1, W12
...
;DPX.c,165 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,166 :: 		case GCU_CLUTCH_FB_SW_ID:
L_CAN_Interrupt25:
;DPX.c,167 :: 		dClutch_injectActualValue((unsigned char)firstInt);
	MOV.B	[W14+0], W10
	CALL	_dClutch_injectActualValue
;DPX.c,168 :: 		d_traction_control_setValueFromCAN(secondInt);
	MOV	[W14+2], W10
...
;DPX.c,178 :: 		dEbb_calibrationState(secondInt);
	MOV	[W14+2], W10
	CALL	_dEbb_calibrationState
;DPX.c,179 :: 		dEbb_error(thirdInt);
	MOV	[W14+4], W10
	CALL	_dEbb_error
;DPX.c,180 :: 		break; //  */
	GOTO	L_CAN_Interrupt18
...
;DPX.c,183 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,184 :: 		case DAU_FL_DEBUG_ID:
L_CAN_Interrupt30:
;DPX.c,185 :: 		dd_Indicator_setIntCoupleValueP(&ind_dau_fl_board.base, (int)firstInt, (int)secondInt);
	MOV	[W14+2], W12
	MOV	[W14+0], W11
...
;DPX.c,186 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,187 :: 		case DAU_REAR_DEBUG_ID:
L_CAN_Interrupt31:
;DPX.c,188 :: 		dd_Indicator_setIntCoupleValueP(&ind_dau_r_board.base, (int)firstInt, (int)secondInt);
	MOV	[W14+2], W12
	MOV	[W14+0], W11
...
;DPX.c,204 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,205 :: 		case DCU_DEBUG_ID:
L_CAN_Interrupt34:
;DPX.c,206 :: 		dd_Indicator_setIntCoupleValueP(&ind_dcu_board.base,(int)firstInt, (int)secondInt);
	MOV	[W14+2], W12
	MOV	[W14+0], W11
