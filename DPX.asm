...
;DPX.c,78 :: 		dClutch_set(dPaddle_getValue());
	...
	MOV.B	W0, W10
	CALL	_dClutch_set
;DPX.c,79 :: 		dClutch_send();
	CALL	_dClutch_send
;DPX.c,80 :: 		timer2_counter2 = 0;
	CLR	W0
...
;DPX.c,88 :: 		}
L_timer2_interrupt7:
;DPX.c,90 :: 		if (timer2_counter3 >= 100) {
	MOV	#100, W1
	MOV	#lo_addr(_timer2_counter3), W0
	CP	W1, [W0]
...
;DPX.c,95 :: 		timer2_counter3 = 0;
	CLR	W0
	MOV	W0, _timer2_counter3
;DPX.c,96 :: 		}
L_timer2_interrupt9:
;DPX.c,98 :: 		if (timer2_counter5 >= 1000) {
...
;DPX.c,112 :: 		onCanInterrupt{
;DPX.c,117 :: 		unsigned int dataLen = 0, flags = 0;
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
...
;DPX.c,121 :: 		dSignalLed_switch(DSIGNAL_LED_RED_RIGHT);
	MOV.B	#1, W10
	CALL	_dSignalLed_switch
;DPX.c,122 :: 		Can_read(&id, dataBuffer, &dataLen, &flags);
	ADD	W14, #22, W3
	ADD	W14, #20, W2
	ADD	W14, #12, W1
	ADD	W14, #8, W0
...
;DPX.c,141 :: 		fourthInt = (unsigned int) ((dataBuffer[6] << 8) | (dataBuffer[7] & 0xFF));
	...
	ADD	W14, #6, W0
	IOR	W2, W1, [W0]
;DPX.c,142 :: 		}                                                        //noi quando mandiamo sw temp e curr?? come lo mostriamo a schermo??
L_CAN_Interrupt16:
;DPX.c,144 :: 		switch (id) {
	GOTO	L_CAN_Interrupt17
;DPX.c,145 :: 		case EFI_GEAR_RPM_TPS_APPS_ID:
L_CAN_Interrupt19:
;DPX.c,146 :: 		dRpm_set(secondInt);
	MOV	[W14+2], W10
	CALL	_dRpm_set
;DPX.c,147 :: 		dEfiSense_heartbeat();
...
;DPX.c,150 :: 		case EFI_WATER_TEMPERATURE_ID:
L_CAN_Interrupt20:
;DPX.c,151 :: 		dd_Indicator_setFloatValueP(&ind_th2o_sx_in.base, dEfiSense_calculateWaterTemperature(firstInt));
	MOV	[W14+0], W10
...
;DPX.c,152 :: 		dd_Indicator_setFloatValueP(&ind_th2o_sx_out.base, dEfiSense_calculateWaterTemperature(secondInt));
	MOV	[W14+2], W10
	CALL	_dEfiSense_calculateWaterTemperature
	MOV	W0, W11
	MOV	W1, W12
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,153 :: 		dd_Indicator_setFloatValueP(&ind_th2o_dx_in.base, dEfiSense_calculateWaterTemperature(thirdInt));
	MOV	[W14+4], W10
	CALL	_dEfiSense_calculateWaterTemperature
	MOV	W0, W11
	MOV	W1, W12
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,154 :: 		dd_Indicator_setFloatValueP(&ind_th2o_dx_out.base, dEfiSense_calculateWaterTemperature(fourthInt));
	MOV	[W14+6], W10
	CALL	_dEfiSense_calculateWaterTemperature
	MOV	W0, W11
	MOV	W1, W12
...
L_CAN_Interrupt21:
;DPX.c,158 :: 		dd_Indicator_setFloatValueP(&ind_oil_temp_in.base, dEfiSense_calculateOilInTemperature(firstInt));
	MOV	[W14+0], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,159 :: 		dd_Indicator_setFloatValueP(&ind_oil_temp_out.base, dEfiSense_calculateOilOutTemperature(secondInt));
	MOV	[W14+2], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,160 :: 		dd_Indicator_setFloatValueP(&ind_th2o.base, dEfiSense_calculateTemperature(thirdInt));
	MOV	[W14+4], W10
	...
	CALL	_dd_Indicator_setFloatValueP
;DPX.c,161 :: 		dd_Indicator_setFloatValueP(&ind_vbat.base, dEfiSense_calculateVoltage(fourthInt));
	MOV	[W14+6], W10
...
;DPX.c,163 :: 		break;
	GOTO	L_CAN_Interrupt18
;DPX.c,164 :: 		case EFI_TRACTION_CONTROL_ID:
L_CAN_Interrupt22:
;DPX.c,165 :: 		break;
	GOTO	L_CAN_Interrupt18
