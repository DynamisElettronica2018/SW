...
;d_drs.c,24 :: 		d_drs_status = DRS_CLOSE;
	...
	CLR	W0
	MOV.B	W0, [W1]
;d_drs.c,25 :: 		dSignalLed_unset(DSIGNAL_LED_GREEN);
	MOV.B	#3, W10
	CALL	_dSignalLed_unset
;d_drs.c,26 :: 		}else if(d_drs_status == DRS_CLOSE && d_drs_feedback == DRS_CLOSE){
...
;d_drs.c,27 :: 		Can_writeInt(SW_DRS_GCU_ID, DRS_OPEN);
	MOV	#1, W12
	MOV	#517, W10
	MOV	#0, W11
	CALL	_Can_writeInt
;d_drs.c,28 :: 		d_drs_status = DRS_OPEN;
	MOV	#lo_addr(_d_drs_status), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
;d_drs.c,29 :: 		dSignalLed_set(DSIGNAL_LED_GREEN);
	MOV.B	#3, W10
	CALL	_dSignalLed_set
;d_drs.c,26 :: 		}else if(d_drs_status == DRS_CLOSE && d_drs_feedback == DRS_CLOSE){
L__d_drs_propagateChange20:
L__d_drs_propagateChange19:
;d_drs.c,31 :: 		Can_writeByte(SW_DRS_GCU_ID, d_drs_lastValue);*/
...
;d_drs.c,34 :: 		void d_drs_setValueFromCAN(unsigned int value){
;d_drs.c,35 :: 		if(d_UI_getOperatingMode() != ACC_MODE){
	PUSH	W11
	PUSH	W12
	PUSH	W10
...
;d_drs.c,36 :: 		if(d_drs_status==value && d_drs_status==DRS_OPEN){
	MOV	#lo_addr(_d_drs_status), W0
	ZE	[W0], W0
	CP	W0, W10
...
;d_drs.c,37 :: 		dd_Indicator_setIntValueP(&ind_drs.base, value);
	PUSH	W10
	MOV	W10, W11
	MOV	#lo_addr(_ind_drs), W10
	CALL	_dd_Indicator_setIntValueP
;d_drs.c,38 :: 		dd_GraphicController_fireTimedNotification(DRS_NOTIFICATION_TIME, "DRS OPEN", MESSAGE);
	CLR	W12
	MOV	#lo_addr(?lstr1_d_drs), W11
	MOV	#500, W10
	CALL	_dd_GraphicController_fireTimedNotification
	POP	W10
;d_drs.c,39 :: 		}else if(d_drs_status==value && d_drs_status==DRS_CLOSE){
...
;d_drs.c,40 :: 		dd_Indicator_setIntValueP(&ind_drs.base, value);
	PUSH	W10
	MOV	W10, W11
	MOV	#lo_addr(_ind_drs), W10
	CALL	_dd_Indicator_setIntValueP
;d_drs.c,41 :: 		dd_GraphicController_fireTimedNotification(DRS_NOTIFICATION_TIME, "DRS CLOSE", MESSAGE);
	CLR	W12
...
;d_drs.c,43 :: 		d_drs_feedback = value;
	MOV	#lo_addr(_d_drs_feedback), W0
	MOV.B	W10, [W0]
;d_drs.c,44 :: 		}
