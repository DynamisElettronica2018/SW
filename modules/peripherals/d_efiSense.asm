...
;d_efiSense.c,45 :: 		case AUTOCROSS_MODE:
L_dEfiSense_getAccValue5:
;d_efiSense.c,46 :: 		if(accValue >= EFI_SENSE_MIN_ACC_VALUE && dAutocross_hasGCUConfirmed() == COMMAND_START_AUTOCROSS){
	MOV	#50, W0
	CP	W10, W0
...
;d_efiSense.c,65 :: 		int dEfiSense_calculateTPS (unsigned int value){
;d_efiSense.c,66 :: 		return ((int)(value*100)/EFI_SENSE_TPS_RANGE);
	MOV	#100, W0
	MUL.UU	W10, W0, W4
	MOV	#255, W2
	REPEAT	#17
...
;d_efiSense.c,69 :: 		float dEfiSense_calculateOilInTemperature (unsigned int value){
;d_efiSense.c,70 :: 		return ((int) (( EFI_SENSE_OIL_MIN_TEMP - (value * EFI_SENSE_OIL_TEMP_RANGE ) ) * 100)) / 100.0;
	MOV	W10, W0
	CLR	W1
	CALL	__Long2Float
	MOV	#52513, W2
...
;d_efiSense.c,73 :: 		float dEfiSense_calculateOilOutTemperature (unsigned int value){
;d_efiSense.c,74 :: 		return dEfiSense_calculateWaterTemperature (value);
...
;d_efiSense.c,77 :: 		float dEfiSense_calculateWaterTemperature (unsigned int value) {
;d_efiSense.c,78 :: 		return ((int) (( EFI_SENSE_WATER_MIN_TEMP - (value * EFI_SENSE_WATER_TEMP_RANGE ) ) * 100)) / 100.0;
	MOV	W10, W0
	CLR	W1
	CALL	__Long2Float
	MOV	#8431, W2
...
;d_efiSense.c,81 :: 		float dEfiSense_calculateTemperature(unsigned int value) { //Value is Temperature, 256 values ranging from -10° to 160°
;d_efiSense.c,82 :: 		return ((int) ((((value * EFI_SENSE_TEMP_RANGE) / 256.0) - EFI_SENSE_MIN_TEMP) * 100)) / 100.0;
	MOV	#160, W0
	MUL.UU	W10, W0, W0
	CLR	W1
	CALL	__Long2Float
...
;d_efiSense.c,85 :: 		float dEfiSense_calculatePressure(unsigned int value) { //Value is Pressure in millibars
;d_efiSense.c,86 :: 		return (value / 10) / 100.0;
	MOV	#10, W2
	REPEAT	#17
	DIV.U	W10, W2
	CLR	W1
...
;d_efiSense.c,89 :: 		float dEfiSense_calculateVoltage(unsigned int value) { //Value is Battery Voltage, 1024 values ranging from 0 to 18V
;d_efiSense.c,90 :: 		return ((int) (((value * EFI_SENSE_MAX_VOLTAGE) / 1024.0) * 100)) / 100.0;
	MOV	#18, W0
	MUL.UU	W10, W0, W0
	CLR	W1
	CALL	__Long2Float
...
;d_efiSense.c,93 :: 		int dEfiSense_calculateSlip(unsigned int value){
;d_efiSense.c,94 :: 		return ((int) ((value * EFI_SENSE_SLIP) * 100)) / 100.0;
	MOV	W10, W0
	CLR	W1
	CALL	__Long2Float
	MOV	#52429, W2
