...
;dd_dashboard.c,59 :: 		void dd_Dashboard_init() {
;dd_dashboard.c,60 :: 		}
...
;dd_dashboard.c,62 :: 		unsigned char dd_Dashboard_getIndicatorIndexAtPosition(DashboardPosition position) {
;dd_dashboard.c,63 :: 		return (unsigned char) position;
	MOV.B	W10, W0
;dd_dashboard.c,64 :: 		}
...
;dd_dashboard.c,69 :: 		void dd_Dashboard_printGearLetter(void) {
;dd_dashboard.c,70 :: 		unsigned char newLetter = dGear_getCurrentGearLetter();
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
	...
	MOV.B	W0, [W14+0]
;dd_dashboard.c,71 :: 		if (newLetter!=dd_lastPrintedGearLetter || dd_GraphicController_isFrameUpdateForced()) {
...
;dd_dashboard.c,72 :: 		eGlcd_setFont(DD_Gears_Font);
	MOV	#48, W13
	MOV.B	#59, W12
	MOV.B	#40, W11
	MOV	#lo_addr(dd_dashboard_DynamisFont_Gears40x59), W10
	CALL	_xGlcd_Set_Font
;dd_dashboard.c,73 :: 		eGlcd_overwriteChar(dd_lastPrintedGearLetter, newLetter, GEAR_LETTER_X, GEAR_LETTER_Y);
...
;dd_dashboard.c,74 :: 		dd_lastPrintedGearLetter = newLetter;
	MOV	#lo_addr(dd_dashboard_dd_lastPrintedGearLetter), W1
	MOV.B	[W14+0], W0
	MOV.B	W0, [W1]
;dd_dashboard.c,75 :: 		}
...
;dd_dashboard.c,78 :: 		/*static*/ unsigned char dd_Indicator_getLabelYCoord(unsigned char indicatorIndex) {
;dd_dashboard.c,79 :: 		return (unsigned char) (DASHBOARD_POSITION_COORDINATES[indicatorIndex][Y] +
	ZE	W10, W0
	SL	W0, #1, W1
	...
	MOV	WREG, 52
	MOV.B	[W1], W0
;dd_dashboard.c,80 :: 		((INDICATOR_HEIGHT - (INDICATOR_RADIUS + INDICATOR_FONT_HEIGHT)) / 2) +
	ZE	W0, W0
	ADD	W0, #9, W0
;dd_dashboard.c,81 :: 		(INDICATOR_RADIUS + INDICATOR_FONT_HEIGHT) - (DASHBOARD_FONT_HEIGHT / 2) +
	ADD	W0, #11, W0
	SUB	W0, #8, W0
;dd_dashboard.c,82 :: 		INDICATOR_MARGIN);
	INC	W0
;dd_dashboard.c,83 :: 		}
...
;dd_dashboard.c,85 :: 		/*static*/ unsigned char dd_Indicator_getLabelXCoord(unsigned char indicatorIndex) {
;dd_dashboard.c,86 :: 		unsigned char length = dd_currentIndicators[indicatorIndex]->labelLength;
	ZE	W10, W0
	SL	W0, #1, W1
	...
	MOV	[W0], W0
	ADD	W0, #9, W3
;dd_dashboard.c,87 :: 		return (unsigned char) ((DASHBOARD_POSITION_COORDINATES[indicatorIndex][X] + INDICATOR_WIDTH / 2) -
	MOV	#lo_addr(dd_dashboard_DASHBOARD_POSITION_COORDINATES), W0
	ADD	W0, W1, W1
	MOV	#___Lib_System_DefaultPage, W0
	MOV	WREG, 52
	...
	ZE	W0, W0
	ADD	W0, #20, W2
;dd_dashboard.c,88 :: 		(length * INDICATOR_FONT_WIDTH) / 2 );
	ZE	[W3], W1
	MOV	#6, W0
	MUL.SS	W1, W0, W0
	ASR	W0, #1, W0
...
;dd_dashboard.c,91 :: 		/*static*/ unsigned char dd_Indicator_getNameXCoord(unsigned char indicatorIndex) {
;dd_dashboard.c,92 :: 		unsigned char length = dd_currentIndicators[indicatorIndex]->nameLength;
	ZE	W10, W0
	SL	W0, #1, W1
	MOV	#lo_addr(_dd_currentIndicators), W0
	ADD	W1, [W0], W0
	MOV	[W0], W0
	ADD	W0, #6, W3
;dd_dashboard.c,93 :: 		return (unsigned char) (DASHBOARD_POSITION_COORDINATES[indicatorIndex][X] +
	MOV	#lo_addr(dd_dashboard_DASHBOARD_POSITION_COORDINATES), W0
	ADD	W0, W1, W1
	MOV	#___Lib_System_DefaultPage, W0
	MOV	WREG, 52
	MOV.B	[W1], W0
;dd_dashboard.c,94 :: 		INDICATOR_WIDTH / 2 - ( length * INDICATOR_FONT_WIDTH) / 2);
	ZE	W0, W0
	ADD	W0, #20, W2
	ZE	[W3], W1
	MOV	#6, W0
...
;dd_dashboard.c,97 :: 		/*static*/ unsigned char dd_Indicator_getNameYCoord(unsigned char indicatorIndex) {
;dd_dashboard.c,98 :: 		return (unsigned char) (DASHBOARD_POSITION_COORDINATES[indicatorIndex][Y] +
	ZE	W10, W0
	SL	W0, #1, W1
	MOV	#lo_addr(dd_dashboard_DASHBOARD_POSITION_COORDINATES), W0
	ADD	W0, W1, W0
	...
	MOV	WREG, 52
	MOV.B	[W1], W0
;dd_dashboard.c,99 :: 		INDICATOR_RADIUS);
	ZE	W0, W0
	ADD	W0, #3, W0
;dd_dashboard.c,100 :: 		}
...
;dd_dashboard.c,105 :: 		/*static*/ void dd_Indicator_drawContainers(unsigned char indicatorIndex) {
;dd_dashboard.c,107 :: 		x = DASHBOARD_POSITION_COORDINATES[indicatorIndex][X];
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
	...
	MOV	WREG, 52
	MOV.B	[W1], W2
;dd_dashboard.c,108 :: 		y = DASHBOARD_POSITION_COORDINATES[indicatorIndex][Y];
	INC	W1
	MOV	#___Lib_System_DefaultPage, W0
	MOV	WREG, 52
	MOV.B	[W1], W0
;dd_dashboard.c,110 :: 		eGlcd(eGlcd_drawRect(
	ZE	W0, W0
	ADD	W0, #1, W1
	ZE	W2, W0
	INC	W0
...
;dd_dashboard.c,122 :: 		static void dd_Dashboard_printIndicator(unsigned char indicatorIndex) {
;dd_dashboard.c,123 :: 		Indicator* indicator = dd_currentIndicators[indicatorIndex];
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
	...
	MOV	[W0], W0
	MOV	W0, [W14+0]
;dd_dashboard.c,126 :: 		if (dd_GraphicController_isFrameUpdateForced()) {
	PUSH	W10
	CALL	_dd_GraphicController_isFrameUpdateForced
	POP	W10
	CP0.B	W0
...
;dd_dashboard.c,127 :: 		dd_Indicator_drawContainers(indicatorIndex);
	PUSH	W10
	CALL	_dd_Indicator_drawContainers
	POP	W10
;dd_dashboard.c,128 :: 		x = dd_Indicator_getNameXCoord(indicatorIndex);
	CALL	_dd_Indicator_getNameXCoord
	MOV.B	W0, [W14+2]
;dd_dashboard.c,129 :: 		y = dd_Indicator_getNameYCoord(indicatorIndex);
	CALL	_dd_Indicator_getNameYCoord
	MOV.B	W0, [W14+3]
;dd_dashboard.c,130 :: 		eGlcd_setFont(DD_xTerminal_Font);
	PUSH	W10
	MOV	#32, W13
	MOV.B	#8, W12
	MOV.B	#6, W11
	...
	CALL	_xGlcd_Set_Font
;dd_dashboard.c,131 :: 		eGlcd_writeText(indicator->name, x, y);
	MOV	[W14+0], W0
	INC2	W0
	MOV.B	[W14+3], W12
	MOV.B	[W14+2], W11
...
;dd_dashboard.c,132 :: 		eGlcd_setFont(DD_Dashboard_Font);
	MOV	#32, W13
	MOV.B	#16, W12
	MOV.B	#16, W11
...
;dd_dashboard.c,137 :: 		x = dd_Indicator_getLabelXCoord(indicatorIndex);
	CALL	_dd_Indicator_getLabelXCoord
	MOV.B	W0, [W14+2]
;dd_dashboard.c,138 :: 		y = dd_Indicator_getLabelYCoord(indicatorIndex);
	CALL	_dd_Indicator_getLabelYCoord
	MOV.B	W0, [W14+3]
;dd_dashboard.c,140 :: 		eGlcd_clearText(indicator->label, x, y);
	MOV	[W14+0], W1
	ADD	W1, #10, W1
	PUSH	W10
	MOV.B	W0, W12
	...
	POP	W10
;dd_dashboard.c,142 :: 		if(dd_Indicator_isRequestingUpdate(indicatorIndex))
	PUSH	W10
...
;dd_dashboard.c,144 :: 		dd_Indicator_parseValueLabel(indicatorIndex);
	PUSH	W10
	CALL	_dd_Indicator_parseValueLabel
	POP	W10
;dd_dashboard.c,145 :: 		x = dd_Indicator_getLabelXCoord(indicatorIndex);
	CALL	_dd_Indicator_getLabelXCoord
	MOV.B	W0, [W14+2]
;dd_dashboard.c,146 :: 		dd_Indicator_clearPrintUpdateRequest(indicatorIndex);
	CALL	_dd_Indicator_clearPrintUpdateRequest
;dd_dashboard.c,147 :: 		}
L_dd_dashboard_dd_Dashboard_printIndicator4:
;dd_dashboard.c,148 :: 		eGlcd_writeText(indicator->label, x, y);
	MOV	[W14+0], W0
	ADD	W0, #10, W0
	MOV.B	[W14+3], W12
	MOV.B	[W14+2], W11
...
;dd_dashboard.c,160 :: 		eGlcd_setFont(DD_Dashboard_Font);
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
...
;dd_dashboard.c,164 :: 		dd_Dashboard_printIndicator(index);
	MOV.B	[W14+0], W10
...
;dd_dashboard.c,171 :: 		void dd_Dashboard_print(void) {
;dd_dashboard.c,172 :: 		if(dd_GraphicController_isFrameUpdateForced())
	CALL	_dd_GraphicController_isFrameUpdateForced
	CP0.B	W0
	BRA NZ	L__dd_Dashboard_print36
	GOTO	L_dd_Dashboard_print11
L__dd_Dashboard_print36:
;dd_dashboard.c,173 :: 		eGlcd_clear();
	CALL	_eGlcd_clear
L_dd_Dashboard_print11:
;dd_dashboard.c,174 :: 		dd_Dashboard_printGearLetter();
	CALL	_dd_Dashboard_printGearLetter
;dd_dashboard.c,175 :: 		dd_Dashboard_printIndicators();
