...
; movement_sx end address is: 0 (W0)
L_cn_interrupt15:
;d_controls.c,206 :: 		if(movement_sx){
; movement_sx start address is: 0 (W0)
	CP0.B	W0
	BRA NZ	L__cn_interrupt73
	GOTO	L_cn_interrupt21
L__cn_interrupt73:
;d_controls.c,207 :: 		d_controls_onLeftEncoder(movement_sx);
	PUSH	W1
; movement_sx end address is: 0 (W0)
	MOV.B	W0, W10
...
;d_controls.c,208 :: 		}
L_cn_interrupt21:
;d_controls.c,209 :: 		if(movement_dx){
	CP0.B	W1
	BRA NZ	L__cn_interrupt74
	GOTO	L_cn_interrupt22
L__cn_interrupt74:
;d_controls.c,210 :: 		d_controls_onRightEncoder(movement_dx);
	MOV.B	W1, W10
; movement_dx end address is: 2 (W1)
	CALL	_d_controls_onRightEncoder
;d_controls.c,211 :: 		}
L_cn_interrupt22:
;d_controls.c,213 :: 		_CLEAR_CN_LABEL:
___cn_interrupt__CLEAR_CN_LABEL:
;d_controls.c,214 :: 		clearExternalInterrupt(CN_DEVICE);
	MOV.B	#9, W10
	CALL	_clearExternalInterrupt
;d_controls.c,215 :: 		}
L_end_cn_interrupt:
...
;d_controls.c,234 :: 		onRotarySwitchInterrupt{
;d_controls.c,235 :: 		signed char position = 0;
	PUSH	W10
;d_controls.c,237 :: 		if(d_isCentralSelectorEnabled)
	MOV	#lo_addr(d_controls_d_isCentralSelectorEnabled), W0
	CP0	[W0]
	BRA NZ	L__external176
	GOTO	L_external123
L__external176:
;d_controls.c,239 :: 		delay_ms(30);
	MOV	#4, W8
	MOV	#3392, W7
L_external124:
	DEC	W7
	...
	DEC	W8
	BRA NZ	L_external124
;d_controls.c,240 :: 		Delay_ms(STRANGE_BUTTON_DELAY);
	MOV	#6666, W7
L_external126:
	DEC	W7
	...
	NOP
	NOP
;d_controls.c,241 :: 		expanderPort = ~I2CExpander_readPort(I2C_ADDRESS_ROTARY_SWITCH);
	MOV.B	#66, W10
	CALL	_I2CExpander_readPort
	COM.B	W0
	MOV.B	W0, [W14+0]
;d_controls.c,242 :: 		sprintf(dstr, "Port: %d\r\n", expanderPort);
	ZE	W0, W0
	PUSH	W0
...
;d_controls.c,243 :: 		Debug_UART_Write(dstr);
	MOV	#lo_addr(_dstr), W10
	CALL	_Debug_UART_Write
;d_controls.c,244 :: 		if (expanderPort == 0) {
	MOV.B	[W14+0], W0
	CP.B	W0, #0
	BRA Z	L__external177
	GOTO	L_external128
L__external177:
;d_controls.c,245 :: 		position = CRUISE_MODE_POSITION;
	CLR	W0
	MOV.B	W0, [W14+1]
;d_controls.c,246 :: 		sprintf(dstr, "Position: %d\r\n", position);
	CLR	W0
	PUSH	W0
	...
	CALL	_sprintf
	SUB	#6, W15
;d_controls.c,247 :: 		Debug_UART_Write(dstr);
	MOV	#lo_addr(_dstr), W10
	CALL	_Debug_UART_Write
;d_controls.c,248 :: 		}
	GOTO	L_external129
L_external128:
;d_controls.c,250 :: 		position = log2(expanderPort) - ROTARY_SWITCH_CENTRAL_POSITION;
	MOV.B	[W14+0], W10
	CALL	_log2
	ADD	W14, #1, W1
	SUB.B	W0, #3, [W1]
L_external129:
;d_controls.c,251 :: 		d_controls_onSelectorSwitched(position);
	MOV.B	[W14+1], W10
	CALL	_d_controls_onSelectorSwitched
;d_controls.c,252 :: 		}
L_external123:
;d_controls.c,253 :: 		clearExternalInterrupt(ROTARY_SWITCH_INTERRUPT);
	MOV.B	#5, W10
	CALL	_clearExternalInterrupt
;d_controls.c,254 :: 		}
L_end_external1:
...
;d_controls.c,256 :: 		onStartInterrupt{
...
L_external330:
	...
	NOP
	NOP
;d_controls.c,258 :: 		d_controls_onStart();
	CALL	_d_controls_onStart
;d_controls.c,259 :: 		clearExternalInterrupt(START_INTERRUPT);
	MOV.B	#7, W10
	CALL	_clearExternalInterrupt
;d_controls.c,260 :: 		}
L_end_external3:
...
;d_controls.c,262 :: 		onDRSInterrupt{
;d_controls.c,263 :: 		Delay_ms(STRANGE_BUTTON_DELAY);
	PUSH	W10
	MOV	#6666, W7
L_external232:
	DEC	W7
	...
	NOP
	NOP
;d_controls.c,264 :: 		if (DRS_BUTTON_PIN == BUTTON_ACTIVE_STATE) {
	BTSC	RD9_bit, BitPos(RD9_bit+0)
	GOTO	L_external234
;d_controls.c,265 :: 		d_controls_onDRS();
	CALL	_d_controls_onDRS
;d_controls.c,266 :: 		}
L_external234:
;d_controls.c,267 :: 		clearExternalInterrupt(DRS_INTERRUPT);
	MOV.B	#6, W10
	CALL	_clearExternalInterrupt
;d_controls.c,268 :: 		}
L_end_external2:
...
;d_controls.c,270 :: 		onGeneralButtonInterrupt{
;d_controls.c,271 :: 		Delay_ms(STRANGE_BUTTON_DELAY);
	PUSH	W10
	MOV	#6666, W7
L_external435:
	DEC	W7
	...
	NOP
	NOP
;d_controls.c,272 :: 		if (NEUTRAL_BUTTON_PIN == BUTTON_ACTIVE_STATE) {
	BTSC	RC13_bit, BitPos(RC13_bit+0)
	GOTO	L_external437
;d_controls.c,273 :: 		d_controls_onNeutral();
	CALL	_d_controls_onNeutral
;d_controls.c,274 :: 		}
	GOTO	L_external438
L_external437:
;d_controls.c,275 :: 		else if (RESET_BUTTON_PIN == BUTTON_ACTIVE_STATE) {
	BTSC	RC14_bit, BitPos(RC14_bit+0)
	GOTO	L_external439
;d_controls.c,276 :: 		d_controls_onReset();
	CALL	_d_controls_onReset
;d_controls.c,277 :: 		}
	GOTO	L_external440
L_external439:
;d_controls.c,278 :: 		else if (AUX_1_BUTTON_PIN == BUTTON_ACTIVE_STATE) {
	BTSC	RD1_bit, BitPos(RD1_bit+0)
	GOTO	L_external441
;d_controls.c,279 :: 		d_controls_onStartAcquisition();
	CALL	_d_controls_onStartAcquisition
;d_controls.c,280 :: 		}
	GOTO	L_external442
L_external441:
;d_controls.c,281 :: 		else if (AUX_2_BUTTON_PIN == BUTTON_ACTIVE_STATE) {
	BTSC	RB15_bit, BitPos(RB15_bit+0)
	GOTO	L_external443
;d_controls.c,282 :: 		d_controls_onAux2();
...
L_external440:
L_external438:
;d_controls.c,284 :: 		clearExternalInterrupt(GENERAL_BUTTON_INTERRUPT);
	MOV.B	#8, W10
	CALL	_clearExternalInterrupt
;d_controls.c,285 :: 		}
L_end_external4:
...
;d_controls.c,289 :: 		void d_controls_onGearUp() {
;d_controls.c,290 :: 		dGear_requestGearUp();
...
;d_controls.c,293 :: 		void d_controls_onGearDown() {
;d_controls.c,294 :: 		dGear_requestGearDown();
...
;d_controls.c,297 :: 		void d_controls_onStart() {
;d_controls.c,298 :: 		if (getExternalInterruptEdge(START_INTERRUPT) == NEGATIVE_EDGE) {
	PUSH	W10
	MOV.B	#7, W10
...
;d_controls.c,299 :: 		dSignalLed_set(DSIGNAL_LED_2);
	MOV.B	#2, W10
	CALL	_dSignalLed_set
;d_controls.c,300 :: 		dStart_switchOn();
	CALL	_dStart_switchOn
;d_controls.c,301 :: 		switchExternalInterruptEdge(START_INTERRUPT);
	MOV.B	#7, W10
	CALL	_switchExternalInterruptEdge
;d_controls.c,302 :: 		} else {
	GOTO	L_d_controls_onStart45
L_d_controls_onStart44:
;d_controls.c,303 :: 		dStart_switchOff();
	CALL	_dStart_switchOff
;d_controls.c,304 :: 		switchExternalInterruptEdge(START_INTERRUPT);
	MOV.B	#7, W10
	CALL	_switchExternalInterruptEdge
;d_controls.c,305 :: 		}
...
;d_controls.c,308 :: 		void d_controls_onNeutral() {
;d_controls.c,309 :: 		if (!dGear_isNeutralSet()) {
	PUSH	W10
	PUSH	W11
	PUSH	W12
...
;d_controls.c,310 :: 		if (dGear_get() == 1) {
	CALL	_dGear_get
	CP.B	W0, #1
	BRA Z	L__d_controls_onNeutral87
	GOTO	L_d_controls_onNeutral47
L__d_controls_onNeutral87:
;d_controls.c,311 :: 		Can_writeInt(SW_GEARSHIFT_ID, GEAR_COMMAND_NEUTRAL_UP);
	MOV	#50, W12
	MOV	#512, W10
	MOV	#0, W11
	CALL	_Can_writeInt
;d_controls.c,312 :: 		} else if (dGear_get() == 2) {
...
;d_controls.c,313 :: 		Can_writeInt(SW_GEARSHIFT_ID, GEAR_COMMAND_NEUTRAL_DOWN);
	MOV	#100, W12
	MOV	#512, W10
	MOV	#0, W11
	CALL	_Can_writeInt
...
;d_controls.c,318 :: 		void d_controls_onReset() {
;d_controls.c,319 :: 		dHardReset_reset();
...
;d_controls.c,322 :: 		void d_controls_onDRS() {
;d_controls.c,323 :: 		}
...
;d_controls.c,325 :: 		void d_controls_onAux2(void) {
;d_controls.c,326 :: 		switch(d_currentOperatingMode){
	GOTO	L_d_controls_onAux250
;d_controls.c,327 :: 		case ACC_MODE:                      //ha senso perchè nell'acceleration mode, il DRS è settato da GCU totalmente aperto
L_d_controls_onAux252:
;d_controls.c,328 :: 		dAcc_requestAction();
	CALL	_dAcc_requestAction
;d_controls.c,329 :: 		default:
L_d_controls_onAux253:
;d_controls.c,330 :: 		return;
...
;d_controls.c,334 :: 		void d_controls_onStartAcquisition(void) {
;d_controls.c,335 :: 		dDCU_switchAcquisition();
